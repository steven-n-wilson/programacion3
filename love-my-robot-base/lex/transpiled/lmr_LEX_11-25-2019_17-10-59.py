...
var = []
r = {'lmr': 'SAY HelloxCOUNT 3'}
try:
    lista = r["lmr"]
    var = lista.split("x")
except:
    print("[EXCEPT] Dictionary was not operable")
...
def cubeLights(robot: cozmo.robot.Robot):
    ...
    if cube1 is not None:
        cube1.set_lights(getattr(lights,new))
    else:
        cozmo.logger.warning("Cozmo is not connected to a LightCube1Id cube - check the battery.")

    if cube2 is not None:
        cube2.set_lights(getattr(lights,new))
    else:
        cozmo.logger.warning("Cozmo is not connected to a LightCube2Id cube - check the battery.")

    if cube3 is not None:
        cube3.set_lights(getattr(lights,new))
    else:
...
def home():
    ...
    return print("CODE RAN")

try:
    home()

except:
    print("[EXCEPT] CODE was not able to RUN")
